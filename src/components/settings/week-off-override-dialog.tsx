...
import { upsertEmployeeWeekOffOverride } from "@/actions/week-off";
...
export function WeekOffOverrideDialog({ employees, initialEmployeeId, initial, onClose }: Props) {
  ...
  async function handleSave() {
    ...
    setSaving(true);
    const r = await upsertEmployeeWeekOffOverride({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="w-full max-w-md rounded-xl border border-border bg-background p-5 shadow-xl space-y-3">
        <p className="text-sm font-semibold">Employee week-off override</p>
